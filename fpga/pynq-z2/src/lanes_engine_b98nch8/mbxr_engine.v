...
//   2      cfg   0   {wpack[48], P[47:32], Q[31:16],         {astride[63:48], wbuf[41], abuf[40], wbase[25:16], abase[9:0]}
//                     G[15:0]}                              (G is the ACTIVATION group count; wpack: 6-bit weight codes)
//   3      mm    0   -                                       -
...
//
//   THE TWO LANES (this copy only; see merge/mbxr_lanes.v and MD5SUMS.src).  MAGIC 0x5A5A0029
//   is 0x5A5A0028 plus these three instructions and the logic behind them; the BlackBox port
//   list, the elaborated SoC, the top level and the XDC are byte for byte 0028's.
//
//   9      lcfg  0   {lane[2:0], local[12:0]} in rs1[15:0]   cfg word
//   10     lgo   0   1 = attention unit, 2 = normalisation   -
//   11     lst   1   -                                       -        rd = lane status
...
module mbxr_engine_core #(
  ...
  wire lcf = c && (cmd_funct == 7'd9);
  wire lgc = c && (cmd_funct == 7'd10);
  wire lsr = c && (cmd_funct == 7'd11);
  ...
  reg          t_abuf, t_wbuf;
  /* THE WEIGHT GRID, per dispatch and not per build.  cfg rs1[48] says this tile's weight
   * plane rows hold 6-bit codes, four to three bytes.  It is a cfg bit and not an `ld` bit
   * because the grid is a property of the TILE's geometry -- the sequencer's phase and the
   * read port's select -- and cfg already carries the rest of that geometry; `ld` moves
   * bytes and does not care what they mean.  A driver that never sets it emits, and gets,
   * exactly the machine that was measured: every register below is 0. */
  reg          t_wp;
  reg [31:0]   q_mult;
  reg [5:0]    q_shift;
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      t_ab <= {AW{1'b0}}; t_wb <= {AW{1'b0}}; t_abuf <= 1'b0; t_wbuf <= 1'b0; t_wp <= 1'b0;
  ...
  mbxr_tseq #(.NCH(NCH), .AW(AW)) u_ts (
    ...
    .act_base(t_ab), .wgt_base(t_wb), .hold(hold), .wpack(t_wp),
    .a_addr(a_addr), .w_addr(w_addr),
    .s0_valid(s0_valid), .s0_clr(s0_clr), .s0_last(s0_last), .s0_ph(s0_ph), .busy(t_busy));

  // mbxd_spad's address is {bank-within-group, word}: bank = {buffer, top word bit}
  // ---- the lanes (merge/mbxr_lanes.v): they own the scratchpad, the MAC and the drain
  // while `own` is non-zero, and every mux below selects the engine when it is 0.
  wire [NRD*16-1:0] ln_rd_addr;
  wire              ln_rd_own, ln_out_own, ln_busy, ln_go_bad;
  wire              ln_s0_valid, ln_s0_clr, ln_s0_last, ln_act_sel;
  wire [63:0]       ln_p_rdata, ln_out_data;
  wire              ln_out_valid;
  wire [31:0]       ln_status;
  wire [32*NCH-1:0] acc;

  wire lane_go_ok = lgc && !ln_busy && !t_busy;
  ...
  generate
    for (gp = 1; gp < NRD; gp = gp + 1) begin : g_wport
      assign rd_addr[gp*16 +: 16] = ln_rd_own ? ln_rd_addr[gp*16 +: 16]
                                              : {5'd0, t_wbuf, w_addr};
    end
  endgenerate
  ...
  reg s2_final;
  wire m_s0_valid = ln_rd_own ? ln_s0_valid : s0_valid;
  wire m_s0_clr   = ln_rd_own ? ln_s0_clr   : s0_clr;
  wire m_s0_last  = ln_rd_own ? ln_s0_last  : s0_last;
  always @(posedge clk) begin
    if (rst) begin
      s1_valid <= 1'b0; s1_clr <= 1'b0; s1_last <= 1'b0; s2_final <= 1'b0;
      s1_ph <= 2'd0; s1_wp <= 1'b0;
    end else begin
      s1_valid <= m_s0_valid;
      s1_clr   <= m_s0_clr;
      s1_last  <= m_s0_last;
      s2_final <= s1_valid && s1_last;
      // the block RAM read is registered, so the phase that ADDRESSED a word reaches the
      // unpacker with it; a lane owning the read ports is never a packed dispatch
      s1_ph    <= s0_ph;
      s1_wp    <= t_wp && !ln_rd_own;
    end
    s1_sel <= ln_rd_own && ln_act_sel;
  end

  // the block RAM read is registered, so the attention unit's activation select is too
  wire [63:0] a_word = s1_sel ? ln_p_rdata : rd_data[63:0];
  // THE ONE EXPRESSION THIS LEVER CHANGES.  `.w(rd_data[64*NRD-1:64])` became a module: at
  // eight bits it is a wire (pack = 0 selects w_raw), at six it is the 48-bit select.
  wire [64*NCH-1:0] w_word;
  mbxr_wunpack #(.NCH(NCH)) u_wu (
    .clk(clk), .valid(s1_valid), .clr(s1_clr), .pack(s1_wp), .ph(s1_ph),
    .w_raw(rd_data[64*NRD-1:64]), .w(w_word));
  mbxr_mac #(.NCH(NCH)) u_mac (
    .clk(clk), .valid(s1_valid), .clr(s1_clr),
    .a(a_word), .w(w_word), .acc(acc));
  ...
  mbxr_lanes #(.NCH(NCH), .AW(AW)) u_lanes (
    ...
    .out_valid(ln_out_valid), .out_data(ln_out_data), .out_hold(hold),
    .out_own(ln_out_own), .busy(ln_busy), .go_bad(ln_go_bad), .status(ln_status));

  wire              qv, q_busy;
  wire [8*NCH-1:0]  qbytes;
  mbxr_quant #(.NCH(NCH)) u_q (
    .clk(clk), .rst(rst), .in_valid(s2_final && !ln_out_own), .acc(acc),
  ...
  mbxr_st #(.DEPTH(SDEPTH), .LGFIFO(5), .MARGIN(4)) u_st (
    ...
    .in_valid(ln_out_own ? ln_out_valid : pk_valid),
    .in_data (ln_out_own ? ln_out_data  : pk_word),
  ...
  always @(posedge clk) begin
    if (rst || clr_stats) begin
      ...
      if (wh_err || (ad_valid && ad_error) || (la_we && pa_bad) ||
          s_ovf || s_dbad || ld_mode_bad || ld_buf_bad || mm_buf_bad || lane_go_bad || ln_go_bad)
        err_sticky <= 1'b1;
    end
  end
  ...
  assign busy = l_busy || t_busy || s_busy || pipe_busy || ln_busy;
  ...
  assign resp_valid = (fen || sta || lsr) && cmd_xd;
  assign resp_data  = lsr ? {32'd0, ln_status} : (fen ? status : {16'd0, stat_sel});
endmodule
...
module mbxr_whalf #(
  ...
  // THE PORT INDEX IS FOUR BITS, AND AT NCH = 4 THAT IS THE SAME HARDWARE.  It was three until
  // 2026-09-19.  Ports run 1..NCH, so at NCH = 8 plane 7 addresses port 8 -- which does not fit
  // three bits and wrapped to 0, whose banks only the ACTIVATION write port drives (mbxd_spad2,
  // the `p == 0` branch), so plane 7's words matched no bank and were SILENTLY DISCARDED.
  // `pbad` cannot catch it: the plane index is legal, it is the port number that overflows, and
  // a 3-bit `+ 1` that wraps is legal Verilog with no warning -- not even [Synth 8-689].
  // 0x5A5A0033 was built, closed at +0.208 and booted with this defect; a matched -DMBXR_NCH=8
  // guest then returned n % 8 == 7 as zero with every runtime gate green (TODO.md, B98).
  // At NCH <= 4 the top bit is always 0 and wbank is bit-identical, which tb_mbxr checks to the
  // Get, the Put and the cycle -- 0x5A5A0032 is not regressed by this line.
  wire [3:0]    pport = pr[3:0] + 4'd1;
  wire          pbad  = (pr >= NCH);
  assign ww_word = {1'd0, pport, fbuf, poff};
