...
//
// FROZEN: the LANE engine as 0x5A5A0029, 002A, 002B, 002C and 002D carry it -- revision 2a
// plus the attention, LayerNorm and LUT lanes, with the FLAT `st` descriptor.  Do not develop
// here; merge/mbxr_engine.v is the live copy.
//
// WHY THIS ONE TOO, and not just the no-lane engine next to it.  The lane DISPATCHERS
// (mbxr_lanes.h, mbxr_lane_dispatch.h) issue `st` themselves, and three workstreams run on
// 002C/002D right now.  The no-lane compat engine cannot exercise them -- it has no lanes -- so
// without this file "the lane paths still work on the silicon people are using" would be a
// claim about a line of code rather than a result.
//
//
...
module mbxr_engine_core #(
  ...
  mbxr_st #(.DEPTH(SDEPTH), .LGFIFO(5), .MARGIN(4)) u_st (
    ...
    .busy(s_busy), .left_blocks(s_left), .acked_blocks(s_acked));
  ...
  always @(posedge clk) begin
    if (rst || clr_stats) begin
      ...
      if (wh_err || (ad_valid && ad_error) || (la_we && pa_bad) ||
          s_ovf || ld_mode_bad || ld_buf_bad || mm_buf_bad || lane_go_bad || ln_go_bad)
        err_sticky <= 1'b1;
    end
  end
...
module mbxr_engine #(
  ...
  mbxr_engine_core #(.NCH(NCH), .LDEPTH(LDEPTH), .SDEPTH(SDEPTH)) u_core (
    ...
    .aa_valid(aa_valid), .aa_ready(aa_ready), .aa_put(aa_put), .aa_addr(aa_addr), .aa_source(aa_source),
