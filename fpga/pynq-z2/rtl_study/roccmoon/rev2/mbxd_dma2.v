...
//
// mbxd_dma2 -- rtl_study/rocc/mbxd_dma.v (measured, left as it was) with one change for engine
// revision 2b's weight lane (ROCC_DECOUPLED.md 8.15.5; MEMORY_BANDWIDTH.md 9.9, the abort-drain
// finding): A RESPONSE IS TAKEN ONLY FOR A SOURCE ID THAT IS WAITING FOR ONE.  After a reset
// with Gets outstanding, their late beats arrive on idle IDs; mbxd_dma wrote them into the
// scratchpad and counted them as returns (nret past nissued: the busy test never clears).
// Here they are dropped and `late` pulses.  The guard does not make a short reset safe on its
// own -- a new Get on a reused ID could still take a stale beat -- so the weight half also holds
// new loads until the W port reports quiet (mbxr_engine.v, mbxr_whalf.w_quiet).
//
// And ADDRESS BOUNDS, per request at the point of issue: a Get is issued only for a block
// inside [WIN_LO, WIN_HI).  The first block outside stops the load -- nothing more is issued,
// `oob` pulses (the weight half makes it a sticky error) and the load finishes as refused once
// what was issued has returned.  No multiply and no latency: the address checked is the one
// the row walk would issue.
//
...
module mbxd_dma2 #(
  ...
  parameter [40:0] WIN_LO = 41'h0,             // requests allowed in [WIN_LO, WIN_HI)
  parameter [40:0] WIN_HI = 41'h100_0000_0000
) (
  input  wire        clk,
  input  wire        rst,
  ...
  output wire        late,         // a response beat arrived for an idle source ID: dropped
  output wire        oob           // the next block lies outside the window: the load stops
);
  ...
  wire [40:0] blk_lo = {1'b0, blk_addr[39:LGBEATS+3], {(LGBEATS+3){1'b0}}};
  wire [40:0] blk_hi = blk_lo + {26'd0, BLKB[14:0]};
  wire        in_win = (blk_lo >= WIN_LO) && (blk_hi <= WIN_HI);
  assign oob        = issuing && !in_win;
  assign req_valid  = issuing && have_free && in_win;
  ...
  // a source ID outside [0, DEPTH) indexes past s_busy (4-bit IDs, DEPTH 4): never take it
  wire        src_ok    = ({28'd0, rsp_source} < DEPTH);
  wire        rsp_ok    = rsp_valid && src_ok && s_busy[rsp_source];
  wire [15:0] r_word    = s_word[rsp_source] + {13'd0, s_beat[rsp_source]};
  wire        r_last    = (s_beat[rsp_source] == {LGBEATS{1'b1}});

  assign sp_we   = rsp_ok;
  assign late    = rsp_valid && !(src_ok && s_busy[rsp_source]);
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      // ---- refuse: the block the walk would issue next is outside the window ------------
      if (oob) begin
        issuing <= 1'b0;
      end
