#!/usr/bin/env python3
"""Load the lever-4 bitstream: the bandwidth instrument on a 128-bit TileLink system bus.

Identical to run_rocket_bw.py except for the MAGIC it insists on.  Everything software can
see is register-compatible with 0x5A5A0007 -- the same UART, microphone, GPIO, DRAM window
and instrument registers -- so an image boots on either and cannot tell them apart by
behaving differently.  What differs is the width of every beat on the system bus, and a
lab that ran against the wrong one would report a perfectly well-formed bytes-per-cycle
figure for the other machine.
...
run_rocket.EXPECT_MAGIC = 0x5A5A_000A
