...
module mbxr_engine_core #(
  parameter W_ASYNC = 0,    // 1: the weight half runs on its own clock (revision 2b)
  parameter NCH    = 4,
  ...
  output wire        wh_req,         // toggles when a weight load is accepted (mbxr_wx)
  output wire [39:0] wh_base,
  output wire [15:0] wh_rblk,
  output wire [15:0] wh_rows,
  output wire [31:0] wh_stride,
  output wire        wh_fbuf,        // quasi-static: held from acceptance until FILL clears
  output wire [3:0]  wh_lgpw,
  output wire [3:0]  wh_cap,
  input  wire        wh_done,        // the weight half's completion toggle (its clock)
  input  wire [31:0] wh_beats_gray,  // words delivered by W, Gray-coded (its clock)
  input  wire        wh_err_tog,     // toggles on a TileLink error on W or a word past NCH planes
  input  wire [7:0]  wh_inflight,    // diagnostic
  input  wire        wh_ready,       // the weight half: out of reset and its W port quiet (its clock)
  ...
  wire [47:0] w_beats;
  wire        w_err, w_ready;
  mbxr_wx_core #(.ASYNC(W_ASYNC)) u_wx (
    .clk(clk), .rst(rst), .accept(ld_w), .busy(lw_busy), .clr_count(sta && cmd_rs2[0]),
    .beats(w_beats), .err_pulse(w_err), .inflight(lw_inflight),
    .x_req(wh_req), .x_done(wh_done), .x_beats_gray(wh_beats_gray), .x_err_tog(wh_err_tog),
    .x_inflight(wh_inflight), .x_ready(wh_ready), .lane_ready(w_ready));
  ...
  mbxd_spad2 #(.NRD(NRD), .GRP(4), .DEPTH(512)) u_sp (
    .clk(clk), .rd_en(s0_valid), .rd_addr(rd_addr), .rd_data(rd_data),
  ...
  always @(posedge clk) begin
    if (rst || clr_stats) begin
      ...
      n_fill_beats <= n_fill_beats + {47'd0, la_we};          // activation beats; W's are in w_beats
      ...
      if (w_err || (ad_valid && ad_error) || (la_we && pa_bad) ||
  ...
  always @* begin
    case (cmd_rs1[2:0])
      3'd0: stat_sel = n_fill_beats + w_beats;
  ...
  // A weight-lane error crosses on the same kind of synchroniser as the lane's completion, and
  // can arrive in the same engine cycle that FILL clears; OR the pulse in so the fence never
  // shows FILL clear before the error that ended the load.
  // bit 41: W lane ready (revision 2b; 1 whenever the weight half shares the engine's clock and reset)
  wire [63:0] status = {s_acked, err_sticky || w_err, s_ovf, pk_fill[3:0], w_ready, 1'b0,
...
module mbxr_whalf #(
  ...
  parameter NCH    = 4,
  parameter LDEPTH = 4,
  parameter [40:0] W_WIN_LO = 41'h0,              // the weight window: no Get outside it
  parameter [40:0] W_WIN_HI = 41'h100_0000_0000
) (
  input  wire        clk,            // the weight half's clock (the engine's in 2a)
  input  wire        rst,
  input  wire        w_quiet,        // the W port has no response outstanding (mbxr_wx.v, W_QUIET)
  // from the engine core: the request toggle and the quasi-static load descriptor
  input  wire        req,
  ...
  input  wire [3:0]  lgpw,
  input  wire [3:0]  cap,
  // to the engine core, all registered on `clk`
  output wire        done,
  output wire [31:0] beats_gray,
  output wire        err_tog,
  output wire [7:0]  inflight_o,
  output wire        ready_o,
  // TileLink W master
  ...
  input  wire        wd_error,
  // the weight banks' write port
  output wire        ww_en,
  output wire [15:0] ww_word,
  output wire [63:0] ww_data
);
  localparam AW = 10;
  wire        rv, rr, we, busy, start, pbad, late, oob;
  wire [15:0] word;
  wire [7:0]  inflight;

  mbxr_wx_half #(.ASYNC(W_ASYNC)) u_wx (
    .wclk(clk), .wrst(rst), .x_req(req), .quiet(w_quiet), .x_done(done), .x_beats_gray(beats_gray),
    .x_err_tog(err_tog), .x_inflight(inflight_o), .x_ready(ready_o),
    .start(start), .busy(busy), .beat(we), .err((wd_valid && wd_error) || (we && pbad) || oob),
    .inflight(inflight));

  mbxd_dma2 #(.DEPTH(LDEPTH), .LGBEATS(3), .WIN_LO(W_WIN_LO), .WIN_HI(W_WIN_HI)) u_dma (
    ...
    .busy(busy), .inflight(inflight), .late(late), .oob(oob));

  wire allow = (inflight < {4'd0, cap});
  assign wa_valid = rv && allow;
  assign rr       = allow && wa_ready;

  // THE WRITE DESTINATION IS TAKEN FROM LANE-DOMAIN REGISTERS, not from the live crossing.
  // `fbuf` and `lgpw` are the core's registers read straight into this clock domain: five bits with
  // no synchroniser, and on the routed 2b design 6.163 ns of route against a 10 ns lane period.
  // They are quasi-static -- the core only changes them when it accepts a load, which it cannot do
  // while FILL is set -- and every write below is inside a fill, so the timing contract holds as
  // written.  It holds by ARGUMENT, though, over a five-bit bus that decides where a write LANDS,
  // and a zero-delay bench cannot test it: there, all five bits change on one edge together.  So
  // they are latched here instead, at `start`, by which time the request toggle's own two-flop
  // synchroniser has held them stable for at least two lane cycles.  After this no write's
  // destination depends on a live crossing, the argument stops being load-bearing, and a mixed
  // sample can only ever produce a load that is refused, never one that writes to the wrong plane.
  reg       fbuf_q;
  reg [3:0] lgpw_q;
  always @(posedge clk) begin
    if (rst) begin fbuf_q <= 1'b0; lgpw_q <= 4'd10; end
    else if (start) begin fbuf_q <= fbuf; lgpw_q <= lgpw; end
  end

  // flat word index -> {port 1..NCH, buffer, bank bit, word}
  wire [15:0]   pr    = word >> lgpw_q;
  wire [15:0]   pmask = (16'd1 << lgpw_q) - 16'd1;
  wire [AW-1:0] poff  = word[AW-1:0] & pmask[AW-1:0];
  wire [2:0]    pport = pr[2:0] + 3'd1;
  assign        pbad  = (pr >= NCH);
  assign ww_word = {2'd0, pport, fbuf_q, poff};
  assign ww_en   = we && !pbad;
endmodule

// ---- core + weight half, two clocks (W_ASYNC = 1, revision 2b) or one (W_ASYNC = 0) ----------------
module mbxr_engine_x #(
  parameter W_ASYNC = 1,
  parameter [40:0] W_WIN_LO = 41'h0,
  parameter [40:0] W_WIN_HI = 41'h100_0000_0000,
  parameter NCH    = 4,
  parameter LDEPTH = 4,
  parameter SDEPTH = 2
  ...
  input  wire        clk,
  input  wire        rst,
  input  wire        wclk,           // the weight half and the weight banks' write port
  input  wire        wrst,           // reset synchronised to wclk
  input  wire        w_quiet,        // W port quiet, from the shim's AR/RLAST balance (W_QUIET)
  input  wire        cmd_valid,
  input  wire [6:0]  cmd_funct,
  input  wire [63:0] cmd_rs1,
  input  wire [63:0] cmd_rs2,
  ...
  wire        wh_req, wh_fbuf, wh_done, wh_err_tog, ww_en;
  wire [31:0] wh_beats_gray;
  wire [39:0] wh_base;
  ...
  mbxr_engine_core #(.W_ASYNC(W_ASYNC), .NCH(NCH), .LDEPTH(LDEPTH), .SDEPTH(SDEPTH)) u_core (
    ...
    .wh_req(wh_req), .wh_base(wh_base), .wh_rblk(wh_rblk), .wh_rows(wh_rows), .wh_stride(wh_stride),
    .wh_fbuf(wh_fbuf), .wh_lgpw(wh_lgpw), .wh_cap(wh_cap), .wh_done(wh_done), .wh_beats_gray(wh_beats_gray),
    .wh_err_tog(wh_err_tog), .wh_inflight(wh_inflight), .wh_ready(wh_ready), .ww_clk(wclk), .ww_en(ww_en), .ww_word(ww_word), .ww_data(ww_data),
  ...
  mbxr_whalf #(.W_ASYNC(W_ASYNC), .NCH(NCH), .LDEPTH(LDEPTH), .W_WIN_LO(W_WIN_LO), .W_WIN_HI(W_WIN_HI)) u_whalf (
    .clk(wclk), .rst(wrst), .w_quiet(w_quiet), .req(wh_req), .base(wh_base), .rblk(wh_rblk), .rows(wh_rows),
    .stride(wh_stride), .fbuf(wh_fbuf), .lgpw(wh_lgpw), .cap(wh_cap), .done(wh_done),
    .beats_gray(wh_beats_gray), .err_tog(wh_err_tog), .inflight_o(wh_inflight), .ready_o(wh_ready),
    .wa_valid(wa_valid), .wa_ready(wa_ready),
...
// ---- revision 2a: one clock, revision 1's exact port list -------------------------------------
module mbxr_engine #(
  parameter NCH    = 4,
  parameter LDEPTH = 4,
  parameter SDEPTH = 2
  ...
  mbxr_engine_x #(.W_ASYNC(0), .NCH(NCH), .LDEPTH(LDEPTH), .SDEPTH(SDEPTH)) u (
    .clk(clk), .rst(rst), .wclk(clk), .wrst(rst), .w_quiet(1'b1),
    .cmd_valid(cmd_valid), .cmd_funct(cmd_funct), .cmd_rs1(cmd_rs1), .cmd_rs2(cmd_rs2), .cmd_xd(cmd_xd),
    .resp_valid(resp_valid), .resp_data(resp_data), .busy(busy),
    .wa_valid(wa_valid), .wa_ready(wa_ready), .wa_addr(wa_addr), .wa_source(wa_source),
    .wd_valid(wd_valid), .wd_source(wd_source), .wd_data(wd_data), .wd_error(wd_error),
    .aa_valid(aa_valid), .aa_ready(aa_ready), .aa_put(aa_put), .aa_size(aa_size), .aa_addr(aa_addr), .aa_source(aa_source),
    .aa_data(aa_data), .aa_last(aa_last), .ad_valid(ad_valid), .ad_ack(ad_ack), .ad_source(ad_source),
    .ad_data(ad_data), .ad_error(ad_error));
endmodule
