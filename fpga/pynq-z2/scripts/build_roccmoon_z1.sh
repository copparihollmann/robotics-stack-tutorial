#!/usr/bin/env bash
# Build the full-feature Rocket SoC PLUS the decoupled accelerator as a RoCC on hart 1, for
# the PYNQ-Z1, at 34.4828 MHz.  MAGIC 0x5A5A0010.  ROCC_DECOUPLED.md section 8.
#
...
#
#   scripts/build_roccmoon_z1.sh          full build, through to a bitstream
#   scripts/build_roccmoon_z1.sh synth    stop after synthesis
set -eo pipefail
cd "$(dirname "$0")/.."
...
echo "=== [1/1] full-feature Rocket + the decoupled RoCC engine ($STAGE) ==="
vivado -mode batch -nojournal -nolog -source tcl/build_rocket_roccmoon.tcl -tclargs "$STAGE" \
  2>&1 | tee "$LOG/z1_rocket_roccmoon.log" | tail -8

BUILD=build_rocket_micrgb_roccmoon_z1

if [ "$STAGE" = "synth" ]; then
  grep -q "SYNTH_ONLY_DONE" "$LOG/z1_rocket_roccmoon.log" || {
    echo "FAILED: synthesis"; grep -m3 "^ERROR" "$LOG/z1_rocket_roccmoon.log"; exit 1; }
  grep -E "MIC_PIN|RGB_PIN_SYNTH" "$LOG/z1_rocket_roccmoon.log" || true
  echo "SYNTH_DONE"
  exit 0
fi

grep -q "BITSTREAM_OK" "$LOG/z1_rocket_roccmoon.log" || {
  echo "FAILED: roccmoon build"; grep -m3 "^ERROR" "$LOG/z1_rocket_roccmoon.log"; exit 1; }
...
for entry in $RGB_TABLE; do
  ...
  line=$(grep -F "RGB_PIN: $port -> " "$LOG/z1_rocket_roccmoon.log" | head -1 || true)
  if [ -z "$line" ]; then
