# ---------------------------------------------------------------------------
# Out-of-context area AND timing for the softmax / layer-norm assist candidates.
#
...
#
# Env:
#   NL_ONLY    space-separated subset of labels to run (default: all)
#   NL_OUT     output directory (default: ./ooc_out)
#   NL_PERIOD  clock period in ns (default: 28.999)
# ---------------------------------------------------------------------------
set here [file dirname [file normalize [info script]]]
set outdir [expr {[info exists ::env(NL_OUT)] ? $::env(NL_OUT) : "$here/ooc_out"}]
set period [expr {[info exists ::env(NL_PERIOD)] ? $::env(NL_PERIOD) : 28.999}]
set only   [expr {[info exists ::env(NL_ONLY)] ? $::env(NL_ONLY) : ""}]
file mkdir $outdir

set part xc7z020clg400-1

set rtl [list $here/nonlin_units.v]

# label                 NL_DUT
set duts {
  {null       nl_v_null}
  {exp2       nl_v_exp2}
  {rsqrt_step nl_v_rsqrt}
  {recip_step nl_v_recip}
  {fmul32     nl_v_fmul}
  {fadd32     nl_v_fadd}
  {fmac32     nl_v_fmac}
}
...
foreach dd $duts {
  ...
  puts "=============== NL_RUN $label (dut=$dut) ==============="

  create_project -in_memory -force -part $part
  read_verilog $rtl

  if {[catch {synth_design -top nl_harness -part $part -mode out_of_context \
                -verilog_define "NL_DUT=$dut"} err]} {
    puts "NL_RESULT $label SYNTH_FAILED: $err"
    close_project
    continue
  }
  ...
  if {[llength $paths] == 0} {
    puts "NL_RESULT $label lut=$luts ff=$ffs dsp=$dsps NO_TIMING_PATH"
  } else {
    ...
    puts [format "NL_RESULT %s lut=%s lutlogic=%s lutmem=%s ff=%s dsp=%s bram=%s slack=%.3f dpd=%.3f logic=%s route=%s levels=%s" \
...
close $fh
puts "NL_ALL_DONE period=$period n=[llength $summary] -> $outdir/summary.tsv"
exit
