#!/usr/bin/env bash
# LEVER 2.  Build the full-feature Rocket SoC + the TileLink bandwidth instrument, with
# the MEMORY BUS on its own clock: mbus, the AXI4-to-AXI3 shim and S_AXI_HP0 on FCLK1 at
# 100 MHz while the core, the L2 and everything else stay at 34.4828 MHz.
# MAGIC 0x5A5A0008.
#
# WHY.  MEMORY_BANDWIDTH.md section 2 measured that with lever 1 in place the L2 path
# saturates its 64-bit link at EXACTLY 8.00 B/cycle -- there is no headroom left on that
# path at this clock, so its ceiling IS the clock.  The whole PL runs at 34.4828 MHz
# because of the P-extension's critical path, which is not in the AXI path.
#
# READ report_cdc AFTER THIS BUILD.  pynqz2_memclk.xdc declares the two clocks
# asynchronous, which is correct -- every crossing is a TileLink AsyncQueue or a
# two-flop ASYNC_REG synchroniser -- but it also means an UNSYNCHRONISED crossing would
# no longer show up as a timing failure.  It shows up in report_cdc instead, and this
# script gates on it.
#
...
#
#   scripts/build_bwfast_z1.sh            full build, through to a bitstream
#   scripts/build_bwfast_z1.sh synth      stop after synthesis (the area gate)
set -eo pipefail
cd "$(dirname "$0")/.."
...
echo "=== [1/1] full-feature Rocket + instrument, memory bus on FCLK1 ($STAGE) ==="
vivado -mode batch -nojournal -nolog -source tcl/build_rocket_bwfast.tcl -tclargs "$STAGE" \
  2>&1 | tee "$LOG/z1_rocket_bwfast.log" | tail -8

BUILD=build_rocket_micrgb_bwfast_z1

if [ "$STAGE" = "synth" ]; then
  grep -q "SYNTH_ONLY_DONE" "$LOG/z1_rocket_bwfast.log" || {
    echo "FAILED: synthesis"; grep -m3 "^ERROR" "$LOG/z1_rocket_bwfast.log"; exit 1; }
  grep -E "MIC_PIN|RGB_PIN_SYNTH" "$LOG/z1_rocket_bwfast.log" || true
  echo "SYNTH_DONE"
  exit 0
fi

grep -q "BITSTREAM_OK" "$LOG/z1_rocket_bwfast.log" || {
  echo "FAILED: bwfast build"; grep -m3 "^ERROR" "$LOG/z1_rocket_bwfast.log"; exit 1; }
...
for entry in $RGB_TABLE; do
  ...
  line=$(grep -F "RGB_PIN: $port -> " "$LOG/z1_rocket_bwfast.log" | head -1 || true)
  if [ -z "$line" ]; then
...
grep -q "RGB_PINS_VERIFIED_POST_ROUTE: 6" "$LOG/z1_rocket_bwfast.log" || {
