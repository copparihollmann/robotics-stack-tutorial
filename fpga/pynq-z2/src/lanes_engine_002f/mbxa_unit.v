...
module mbxa_core #(
  ...
  input  wire          acc_valid,    // the engine's s2_final: acc holds a quad's four finals
  // to the drain
  ...
  // A job's Q quads produce 4Q bytes, and nsc / nout must fit inside that.  gp is capped so
  ...
  wire cfg_ok = (c_gs >= 8'd2) && (c_gp >= 8'd2) && (c_gp <= GPMAX) &&
                ...
                ({c_qs[13:0], 2'd0} >= c_nsc) && ({c_qp[13:0], 2'd0} >= c_nout) &&
  ...
  assign ser_hold = sv1;
  assign ser_v    = sv0;
  assign ser_word = (scnt == 3'd4) ? sq0[31:0]  :
                    (scnt == 3'd3) ? sq0[63:32] :
                    (scnt == 3'd2) ? sq0[95:64] : sq0[127:96];

  wire ser_take = sv0 && (scnt == 3'd1);      // the last of the four leaves this cycle
  always @(posedge clk) begin
    if (rst) begin
      sv0 <= 1'b0; sv1 <= 1'b0; scnt <= 3'd0;
    end else begin
      if (ser_take) begin
        if (sv1) begin sq0 <= sq1; sv1 <= 1'b0; scnt <= 3'd4; end
        else if (acc_valid) begin sq0 <= acc; scnt <= 3'd4; end
        else begin sv0 <= 1'b0; scnt <= 3'd0; end
      end else if (sv0) begin
        scnt <= scnt - 3'd1;
        if (acc_valid) begin sq1 <= acc; sv1 <= 1'b1; end
      end else if (acc_valid) begin
        sq0 <= acc; sv0 <= 1'b1; scnt <= 3'd4;
      end
    end
  end
...
module mbxa_unit #(
  ...
  mbxa_core #(.AW(AW), .PRAW(PRAW), .DIV_BPC(DIV_BPC)) u_core (
