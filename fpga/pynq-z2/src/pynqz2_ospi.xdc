# The HM01B0 camera shield's VIDEO pins and timing for PYNQZ2_HAS_OSPI (0x5A5A0038), WITHOUT
# the sensor's I2C.
#
# THIS FILE IS src/pynqz2_cam.xdc WITH ITS TWO I2C LINES REMOVED, and nothing else.  Every ball,
# every IOSTANDARD, the PCLK clock, the input-delay model, the CLOCK_DEDICATED_ROUTE escape, the
# MCLK generated clock and the TRIG/INT false paths are that file's, character for character; a
# diff against it is two deletions.  It exists because src/pynqz2_i2c.xdc already constrains
# P15/P16 for the variant this serves -- the shield's sensor (behind a PCA9306) and its J4 OLED
# row are on the SAME two balls, so there is one TLI2C, one pair of pads and one XDC that owns
# them.  Two files constraining P15/P16 in one build is two ports on one ball.
#
# A SEPARATE constraint file, added by tcl/build_rocket.tcl only when has_ospi is set, for the
...
# TRIG is a software pulse and INT is a level the RTL synchronises (syncSys).  The sensor's I2C
# is NOT here: src/pynqz2_i2c.xdc owns P15/P16 and false-paths them, for this variant and for
# the OLED that shares the bus.
