...
/*
 * add_s16_pc_s8 -- add_pc_s8 with an int16 first operand (extract_q16).  The reference
...
static inline int8_t padd16_clip8(int64_t v)
{
                      ...
                      const int64_t *bmul, int8_t *output, int n, int C)
{
	...
	for (; i + C <= n; i += C) {
		const int16_t *pa = a + i;
		const int8_t *pb = b + i;
		int8_t *po = output + i;

		for (int c = 0; c < C; c++)
			po[c] = padd16_clip8(((int64_t)pa[c] * amul[c] + (int64_t)pb[c] * bmul[c] + rnd) >> 24);
	}
	for (int c = 0; i < n; i++, c++)
		output[i] = padd16_clip8(((int64_t)a[i] * amul[c] + (int64_t)b[i] * bmul[c] + rnd) >> 24);
}
