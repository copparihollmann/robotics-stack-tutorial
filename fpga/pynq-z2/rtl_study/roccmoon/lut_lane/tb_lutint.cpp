...
struct Sim {
  ...
  uint64_t gets = 0, puts = 0, early_bytes = 0;
  ...
  void accept_a(Slave &s, bool fire, bool put, uint64_t addr, int src, uint64_t data, bool last) {
    ...
    if (s.put_src < 0) {
      ...
      s.put_src = src; s.put_addr = addr; s.put_beats = 0;
    } else if (s.put_src != src || s.put_addr != addr) {
      err("Put burst changed source or address mid-burst");
    }
    ...
    s.put_beats++;
    if (last) {
      if (s.put_beats != 8) err("Put burst is not 8 beats");
      if (addr & 63) err("Put not block aligned");
      puts++;
      ...
      if (commit_at_ack)             // the bytes land one cycle before the ack is presented
        for (int b = 0; b < 8; b++) pending.insert({ s.t[src].due - 1, { addr + 8ULL * b, put_buf[src & 15][b] } });
      s.put_src = -1;
    } else if (s.put_beats >= 8) {
      err("Put burst longer than 8 beats");
    }
  }
  ...
  void tick() {
    ...
    top->clk = 0; top->eval();
    top->cmd_valid = 0;
    accept_a(A, af, aa_put, aa_addr, aa_src, aa_data, aa_last);
    sample_live();
    cyc++;
  }
#else
...
static int run_case(Sim &S, mbxr_dev &dev, const Case &c, uint64_t seed, bool verbose, uint64_t in_off) {
  ...
  mbxr_wimage img;
  if (!mbxr_wimage_plan(&img, N, K)) { fprintf(stderr, "%s: plan refused\n", c.name.c_str()); return 1; }
  uint64_t img_pa = MEM_BASE + (32ULL << 20);
  mbxr_wimage_build(&dev, &img, img_pa, w.data(), bias.data());
  uint64_t scratch_pa = MEM_BASE + (64ULL << 20);
  std::vector<int8_t> out((size_t)npix * N, 0x55), gold((size_t)npix * N);
  ...
  uint64_t c0 = S.cyc;
  int rc = mbxr_run(&dev, &img, in_pa, npix, as, &q, scratch_pa, out.data(), &st);
  ...
  for (size_t i = 0; i < out.size(); i++) {
    int e = abs((int)out[i] - (int)gold[i]);
    if (e) { if (bad < 5) fprintf(stderr, "  %s: out[%zu] = %d, reference %d\n", c.name.c_str(), i, out[i], gold[i]); bad++; }
  ...
  if (verbose || bad)
    printf("  %-28s npix %4d K %5d N %5d astride %4d | tiles_w %3d Q %3d lgpw %2d | pairs %4llu "
           ...
           "shift %3d amin %4d amax %4d | early %d placed early %7llu of %7llu | max_abs_err %d%s\n",
           ...
           q.shift, q.amin, q.amax, dev.place_early, (unsigned long long)st.placed_early,
           (unsigned long long)out.size(), maxerr, bad ? "  MISMATCH" : "");
...
// ============================================================================================
// tb_lutint -- THE LUT LANE THROUGH THE MERGED ENGINE, added to a COPY of tb_mbxr.cpp.
//
// tb_mbxr.cpp is the engine workstream's gate and four agents are live in this tree, so this
// is a copy in lut_lane/ rather than an edit -- the same decision the engine workstream took
// when it needed a probe in that file.  Everything above this line is tb_mbxr.cpp byte for
// byte; only `lut_main` and one branch of `main` are new.
//
// WHY IT EXISTS.  Lab B37 arm A put the first dispatch ever issued to mbxl_lut on silicon and
// it HUNG: ownership never came back, and the engine's fence word read MBXR_S_ERR | MBXR_S_OVF
// with s_acked = 128 and s_left = 0.  mbxl_lut's own testbench passes 73,936 byte checks under
// 95 % backpressure, so the fault is in the INTEGRATION -- which no testbench covered, because
// lut_lane/run_tb.sh drives the lane alone and tb_mbxr's --lanes suite drives lanes 2 and 3.
// Reproducing it here costs no board time and gives a waveform.
// ============================================================================================
static const uint64_t LUT_SRC = MEM_BASE + (24ULL << 20);
static const uint64_t LUT_DST = MEM_BASE + (32ULL << 20);

static inline uint8_t lut_tbl_of(uint8_t x) { return (uint8_t)((x * 181u + 71u) & 0xffu); }
static inline uint8_t lut_in_of(uint32_t i) { return (uint8_t)((i * 97u + (i >> 8) * 13u) & 0xffu); }

static void lut_dump(Sim &S, const char *tag) {
  uint32_t st = (uint32_t)S.cmd(11, 0, 0, 1);
  uint64_t en = S.cmd(6, 0, 0, 1);
  printf("  %-14s lane=0x%08x own=%u u_err=0x%x u_idle=%u r_left=%u | engine=0x%016llx "
         "acked=%llu left=%llu ovf=%llu err=%llu busy=%llu\n",
         tag, st, (unsigned)((st >> 30) & 3), (unsigned)((st >> 13) & 7),
         (unsigned)((st >> 12) & 1), (unsigned)(st & 0x7ff),
         (unsigned long long)en, (unsigned long long)((en >> 48) & 0xffff),
         (unsigned long long)((en >> 24) & 0xffff), (unsigned long long)((en >> 46) & 1),
         (unsigned long long)((en >> 47) & 1), (unsigned long long)(en & 0xf));
}

// one LUT dispatch, written exactly as samples/lut_lane_smoke drives it on the board
static int lut_case(Sim &S, int words, int drain_blocks_override, int verbose) {
  lane_reset(S);
  const int bytes = words * 8;
  // the whole activation buffer is filled, so a lane that reads past `words` reads defined
  // data rather than X -- a wrap would then be a WRONG ANSWER and not an X, which is what the
  // board sees too
  for (int i = 0; i < 8192; i++) *S.p(LUT_SRC + i) = lut_in_of((uint32_t)i);
  for (int i = 0; i < 8192 + 128; i++) *S.p(LUT_DST + i) = 0xAA;      // poison

  for (unsigned i = 0; i < 256; i++)
    S.cmd(9, (4ULL << 13) | i, lut_tbl_of((uint8_t)i), 0);

  S.cmd(0, LUT_SRC, (128ULL) | (1ULL << 16), 0);                      // sd: 128 blocks, 1 row
  S.cmd(1, (1ULL << 18) | (0ULL << 16) | (10ULL << 8), 0, 0);         // ld: activations, buf 0
  for (int i = 0; i < 200000; i++) { if (!(S.cmd(6, 0, 0, 1) & 0xf)) break; }
  if (verbose) lut_dump(S, "after fill");

  S.cmd(2, 0, (0ULL << 40), 0);                                       // cfg: t_abuf = 0
  int blocks = drain_blocks_override ? drain_blocks_override : (bytes + 63) / 64;
  // st: the drain.  Since the descriptor became 2-D, a flat drain of `blocks` 64-byte blocks
  // is `blocks` rows of 64 bytes 64 bytes apart -- {stride[63:32], bytes[31:16], nrows[15:0]}.
  // The row count sits in rs2[15:0] because a pre-002E engine reads exactly those bits as
  // nblocks.  This case issues the raw command on purpose (it is samples/lut_lane_smoke's own
  // sequence, not the shared dispatcher's), so the encoding lives here too.
  S.cmd(5, LUT_DST, (64ULL << 32) | (64ULL << 16) | ((uint64_t)blocks & 0xffffULL), 0);
  S.cmd(9, (4ULL << 13) | 0x100, 0, 0);                               // word0
  S.cmd(9, (4ULL << 13) | 0x101, (uint64_t)words, 0);                 // words
  if (verbose) lut_dump(S, "armed");

  S.cmd(10, 3, 0, 0);                                                 // lgo: MBXR_GO_LUT
  uint64_t polls = 0;
  uint32_t st = 0;
  for (; polls < 200000; polls++) {
    st = (uint32_t)S.cmd(11, 0, 0, 1);
    if (((st >> 30) & 3) == 0) break;
    if (verbose && polls < 6) lut_dump(S, "running");
  }
  int hung = (((st >> 30) & 3) != 0);
  if (verbose) lut_dump(S, hung ? "HUNG" : "returned");
  for (int i = 0; i < 200000; i++) { if (!(S.cmd(6, 0, 0, 1) & 0xf)) break; }

  int bad = 0, first = -1;
  for (int i = 0; i < bytes; i++) {
    uint8_t want = lut_tbl_of(lut_in_of((uint32_t)i));
    if (*S.p(LUT_DST + i) != want) { if (first < 0) first = i; bad++; }
  }
  printf("LUT words=%-5d blocks=%-4d %-8s polls=%-8llu u_err=0x%x bad=%d/%d first_bad=%d\n",
         words, blocks, hung ? "HUNG" : "returned", (unsigned long long)polls,
         (unsigned)((st >> 13) & 7), bad, bytes, first);
  return hung || bad;
}

// ============================================================================================
// THE REFACTOR'S GATE: the same dispatches through the SHARED path must be BYTE-IDENTICAL.
//
// mbxr_lane_dispatch.h factors one dispatch path out of the three that exist (attention,
// LayerNorm, and the bench T4 wrote).  A refactor that also fixes things cannot be bisected, so
// the rule it is landed under is that existing lanes produce byte-identical results -- and this
// is that check, on the one lane whose dispatcher this workstream owns.  `lut_case` above is
// the hand-written sequence arm C ran on silicon; `lut_case_shared` is the same dispatch issued
// through the shared helper.  Both byte-check against the same golden table, so a difference in
// EITHER direction fails.
// ============================================================================================
static int g_lut_words = 0;
static int lut_cfg_cb(void *ctx) {
  Sim *S = (Sim *)ctx;
  S->cmd(9, (4ULL << 13) | 0x100, 0, 0);                        // word0
  S->cmd(9, (4ULL << 13) | 0x101, (uint64_t)g_lut_words, 0);    // words
  return MBXR_OK;
}

static int lut_case_shared(Sim &S, int words) {
  lane_reset(S);
  const int bytes = words * 8;
  for (int i = 0; i < 8192; i++) *S.p(LUT_SRC + i) = lut_in_of((uint32_t)i);
  for (int i = 0; i < 8192 + 128; i++) *S.p(LUT_DST + i) = 0xAA;
  for (unsigned i = 0; i < 256; i++)
    S.cmd(9, (4ULL << 13) | i, lut_tbl_of((uint8_t)i), 0);

  mbxr_lane_plan pl;
  pl.src_pa = LUT_SRC; pl.src_blocks = 128; pl.abuf = 0; pl.word0 = 0; pl.words = words;
  pl.dst_pa = LUT_DST; pl.dst_blocks = (bytes + 63) / 64;
  pl.which = MBXR_GO_LUT; pl.budget = 200000;
  mbxr_lane_stats st;
  memset(&st, 0, sizeof st);
  g_lut_words = words;
  int rc = mbxr_lane_dispatch(c_cmd, &S, &pl, lut_cfg_cb, &S, &st);
  ...
  printf("SHARED words=%-5d rc=%-4d lane=%u refused=%u failed=%u u_err=0x%x bad=%d/%d first=%d\n",
         words, rc, st.calls_lane, st.calls_refused, st.calls_failed, st.last_uerr,
         bad, bytes, first);
  return (rc != MBXR_OK) || bad;
}

// and the preconditions the shared path refuses BY NAME, issuing nothing.  Each of these is a
// silent failure on the interface as it stands: -76 returns rc 0 with the last block unwritten
// and the drain busy forever, -2 fills from partway into a block, -70 wraps the buffer.
static int lut_precond(void) {
  struct { mbxr_lane_plan p; int want; const char *what; } t[] = {
    { { LUT_SRC,      128, 0,    0, 1024, LUT_DST,      128, MBXR_GO_LUT, 200000 }, MBXR_OK,
      "the shape arm C ran" },
    { { LUT_SRC,      128, 0,    0,   28, LUT_DST,        4, MBXR_GO_LUT, 200000 },
      MBXR_E_LANE_BLOCKS, "words not a multiple of 8 -- rc 0 and a drain busy forever" },
    { { LUT_SRC + 8,  128, 0,    0, 1024, LUT_DST,      128, MBXR_GO_LUT, 200000 },
      MBXR_E_ALIGN, "src 8 bytes into a block -- checked in ONE of three dispatchers today" },
    { { LUT_SRC,      128, 0,    0, 1024, LUT_DST + 8,  128, MBXR_GO_LUT, 200000 },
      MBXR_E_ALIGN, "dst not 64-byte aligned" },
    { { LUT_SRC,      128, 0, 1017,    8, LUT_DST,        1, MBXR_GO_LUT, 200000 },
      MBXR_E_LANE_SPAN, "the range leaves the buffer -- 0029/002A wrap here silently" },
    { { LUT_SRC,      128, 0,    0, 1024, LUT_DST,      120, MBXR_GO_LUT, 200000 },
      MBXR_E_LANE_DRAIN, "a short drain -- the lane stalls and poisons the stream" },
    { { LUT_SRC,      128, 0,    0, 1024, LUT_DST,      136, MBXR_GO_LUT, 200000 },
      MBXR_E_LANE_DRAIN, "a drain over by a whole block -- it waits forever" },
    { { LUT_SRC,       64, 0,    0, 1024, LUT_DST,      128, MBXR_GO_LUT, 200000 },
      MBXR_E_SHAPE, "the fill covers half the words the lane will read" },
  };
  int bad = 0;
  for (unsigned i = 0; i < sizeof t / sizeof t[0]; i++) {
    int rc = mbxr_lane_check(&t[i].p);
    printf("PRECOND rc=%-4d want=%-4d %-18s %s\n", rc, t[i].want,
           rc == t[i].want ? "ok" : "MISMATCH", t[i].what);
    if (rc != t[i].want) bad++;
  }
  // the attention unit's P-RAM guard, kept with the others because it is the same class
  int a = mbxr_lane_check_pram(0, 64, 7, 512), b = mbxr_lane_check_pram(0, 65, 7, 512);
  printf("PRECOND pram 64*(7+1)=512 rc=%d (want 0) ; 65*8=520 rc=%d (want %d)\n",
         a, b, MBXR_E_LANE_PRAM);
  if (a != MBXR_OK || b != MBXR_E_LANE_PRAM) bad++;
  return bad;
}

static int lut_main(Sim &S) {
  g_lane_sim = &S;
  int fails = 0;
  printf("MBXR_LUT: the integration nothing covers -- mbxl_lut through mbxr_lanes and mbxr_st\n");
  fails += lut_case(S, 256, 0, 1);
  // THE SWEEP: where does it start to fail?  mbxr_st's FIFO is 32 words with almost_full at
  // 28, so if the diagnosis is "out_valid is held rather than pulsed" the boundary must sit
  // just past the point where the drain first backs up -- not at some arbitrary size.
  static const int sweep[] = { 8, 16, 24, 32, 40, 48, 64, 128, 256, 512, 1024 };
  for (unsigned i = 0; i < sizeof sweep / sizeof sweep[0]; i++)
    fails += (lut_case(S, sweep[i], 0, 0) ? 1 : 0);

  // A PRECONDITION NOBODY HAD WRITTEN DOWN, found by this sweep rather than by reading:
  // THE WORD COUNT MUST BE A MULTIPLE OF EIGHT.  `mbxr_st` issues a burst only when a whole
  // 64-byte block is in the FIFO (`have_block = (used >= 8)`, mbxr_st.v:79), so a dispatch of
  // 28 words leaves 4 words that are NEVER written: the destination keeps its previous
  // contents for the last block, the lane goes idle and RETURNS OWNERSHIP CLEANLY, and the
  // DRAIN STAYS BUSY FOREVER -- 88af508's armed-and-unfed poison in a new guise, reached
  // without arming anything wrong.  rc is 0, u_err is 0x0, and 32 of 224 bytes are stale.
  //
  // It bites the real shapes: a GELU op of 190,080 elements is 23 full 1,024-word tiles plus
  // 208 words, and 208 is not a multiple of 8.  The driver must zero-pad the final tile to a
  // whole block exactly as mbxr_ln_dispatch does -- `blocks` rounded up, `words` exact.
  printf("PRECONDITION words %% 8 != 0 (the last partial block is never drained):\n");
  (void)lut_case(S, 28, 0, 0);
  // ---- the refactor's gate ------------------------------------------------------------
  printf("\nSHARED DISPATCH PATH (mbxr_lane_dispatch.h): byte-identical or it does not land\n");
  fails += lut_precond();
  static const int sh[] = { 8, 32, 128, 256, 1024 };
  for (unsigned i = 0; i < sizeof sh / sizeof sh[0]; i++)
    fails += (lut_case_shared(S, sh[i]) ? 1 : 0);

  printf("TileLink: %llu Gets, %llu Puts, %d protocol errors, %llu cycles simulated\n",
         (unsigned long long)S.gets, (unsigned long long)S.puts, S.errors,
         (unsigned long long)S.cyc);
  printf(fails ? "MBXR_LUT_FAIL %d\n" : "MBXR_LUT_OK\n", fails);
  return fails;
}
...
int main(int argc, char **argv) {
  ...
  Sim S;
  int random_cases = 60;
  bool big = true, lanes_only = false, lut_only = false;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(argv[i], "--lut")) lut_only = true;
  }
  if (lut_only) {
  ...
  printf("MBXR engine id %016llx (NCH %llu LDEPTH %llu SDEPTH %llu)\n", (unsigned long long)id,
