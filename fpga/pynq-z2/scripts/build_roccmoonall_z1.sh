#!/usr/bin/env bash
# Build the full-feature Rocket SoC PLUS engine revision 2a, the big core.s pipelined multiplier and 0092.s skipped clean Release, for the PYNQ-Z1,
# at 34.4828 MHz.  MAGIC 0x5A5A0028.  ROCC_DECOUPLED.md section 8.15.9.  build_roccmoon_z1.sh with
...
#
#   scripts/build_roccmoonall_z1.sh          full build, through to a bitstream
#   scripts/build_roccmoonall_z1.sh synth    stop after synthesis
set -eo pipefail
cd "$(dirname "$0")/.."
...
vivado -mode batch -nojournal -nolog -source tcl/build_rocket_roccmoonall.tcl -tclargs "$STAGE" \
  2>&1 | tee "$LOG/z1_rocket_roccmoonall.log" | tail -8

BUILD=build_rocket_micrgb_roccmoonall_z1

if [ "$STAGE" = "synth" ]; then
  grep -q "SYNTH_ONLY_DONE" "$LOG/z1_rocket_roccmoonall.log" || {
    echo "FAILED: synthesis"; grep -m3 "^ERROR" "$LOG/z1_rocket_roccmoonall.log"; exit 1; }
  grep -E "MIC_PIN|RGB_PIN_SYNTH" "$LOG/z1_rocket_roccmoonall.log" || true
  echo "SYNTH_DONE"
  exit 0
fi

grep -q "BITSTREAM_OK" "$LOG/z1_rocket_roccmoonall.log" || {
  echo "FAILED: roccmoon build"; grep -m3 "^ERROR" "$LOG/z1_rocket_roccmoonall.log"; exit 1; }
...
for entry in $RGB_TABLE; do
  ...
  line=$(grep -F "RGB_PIN: $port -> " "$LOG/z1_rocket_roccmoonall.log" | head -1 || true)
  if [ -z "$line" ]; then
