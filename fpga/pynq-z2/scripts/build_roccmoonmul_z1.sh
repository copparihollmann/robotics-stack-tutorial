#!/usr/bin/env bash
# Build the full-feature Rocket SoC PLUS the decoupled accelerator as a RoCC on hart 1 AND the big
# core's pipelined multiplier (mulUnroll = 64 on hart 0), for
# the PYNQ-Z1, at 34.4828 MHz.  MAGIC 0x5A5A0011.  ROCC_DECOUPLED.md section 8.15.3.
#
...
#
#   scripts/build_roccmoonmul_z1.sh          full build, through to a bitstream
#   scripts/build_roccmoonmul_z1.sh synth    stop after synthesis
set -eo pipefail
cd "$(dirname "$0")/.."
...
if [ ! -x "$VERILATOR" ]; then
  ...
  R="$(pwd)/rtl_study"
  # engine revision 1, as this bitstream was built: rtl_study/ holds revision 2a since 2026-09-17
  E="$(pwd)/src/cam_engine_rev1"
  (cd "$E" && md5sum -c --quiet MD5SUMS) || { echo "FAILED: src/cam_engine_rev1 does not match MD5SUMS"; exit 1; }
  rm -rf "$LOG/vmbxr"
  "$VERILATOR" --cc --exe --build -j 8 -O3 -Wno-fatal -DMBXR_BEHAVIOURAL --Mdir "$LOG/vmbxr" \
     --top-module mbxr_engine -CFLAGS "-O2 -I$(pwd)/sw/roccmoon" \
     "$E/mbxr_engine.v" "$E/mbxr_tseq.v" "$E/mbxr_datapath.v" \
     "$E/mbxr_st.v" "$E/mbxd_dma.v" "$E/mbxd_spad.v" "$E/mbx_mac.v" \
...
echo "=== [0f/6] the 0x5A5A0011 RTL gates on this SoC's own TestHarness (scripts/63) ==="
# rv64um riscv-tests on hart 0, the MBP selftest, the RoCC engine check, and the encoder-dispatch
# golden against host C.  A passing log from a run made AFTER this config's gensrc bundle was packed
# is accepted (ROCCMOONMUL_GATES_LOG); otherwise the gates run here.
GSRC="chipyard/gensrc/PynqZ2RocketBigLittlePextTacitMicRgbRoccMoonMulConfig.tar.gz"
if [ -n "${ROCCMOONMUL_GATES_LOG:-}" ] && grep -q "ROCCMOONMUL_RTL_GATES: PASS" "$ROCCMOONMUL_GATES_LOG" \
   && [ "$ROCCMOONMUL_GATES_LOG" -nt "$GSRC" ]; then
  echo "  accepted: $ROCCMOONMUL_GATES_LOG (newer than $GSRC)"
elif [ -z "${CHIPYARD_DIR:-}" ]; then
  echo "FAILED: CHIPYARD_DIR is not set and no passing ROCCMOONMUL_GATES_LOG was given"; exit 1
else
  "$REPO/scripts/63_roccmoonmul_rtl_gates.sh" > "$LOG/roccmoonmul_rtl_gates.log" 2>&1 || {
    echo "FAILED: RTL gates"; tail -30 "$LOG/roccmoonmul_rtl_gates.log"; exit 1; }
  grep -q "ROCCMOONMUL_RTL_GATES: PASS" "$LOG/roccmoonmul_rtl_gates.log" || {
    echo "FAILED: the RTL gates did not report a pass"; exit 1; }
fi

. ./scripts/require_vivado.sh

echo "=== [1/1] full-feature Rocket + the decoupled RoCC engine + pipelined multiplier ($STAGE) ==="
vivado -mode batch -nojournal -nolog -source tcl/build_rocket_roccmoonmul.tcl -tclargs "$STAGE" \
  2>&1 | tee "$LOG/z1_rocket_roccmoonmul.log" | tail -8

BUILD=build_rocket_micrgb_roccmoonmul_z1

if [ "$STAGE" = "synth" ]; then
  grep -q "SYNTH_ONLY_DONE" "$LOG/z1_rocket_roccmoonmul.log" || {
    echo "FAILED: synthesis"; grep -m3 "^ERROR" "$LOG/z1_rocket_roccmoonmul.log"; exit 1; }
  grep -E "MIC_PIN|RGB_PIN_SYNTH" "$LOG/z1_rocket_roccmoonmul.log" || true
  echo "SYNTH_DONE"
  exit 0
fi

grep -q "BITSTREAM_OK" "$LOG/z1_rocket_roccmoonmul.log" || {
  echo "FAILED: roccmoonmul build"; grep -m3 "^ERROR" "$LOG/z1_rocket_roccmoonmul.log"; exit 1; }
...
for entry in $RGB_TABLE; do
  ...
  line=$(grep -F "RGB_PIN: $port -> " "$LOG/z1_rocket_roccmoonmul.log" | head -1 || true)
  if [ -z "$line" ]; then
